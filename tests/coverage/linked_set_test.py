"""
Tests for linked_set python file
"""
...
from magpy.linked_set import LinkedBSFSet, EvenSet, OddSet
...
def test_large_many_body_commute():
    ...
    assert np.array_equal(large_many_commuted_terms,
                          large_many_exp_commuted_terms[0])
    for large_many_map, large_many_exp_map in zip(large_many_maps, large_many_exp_maps):
        assert np.array_equal(large_many_map.todense(),
                              large_many_exp_map.todense())
